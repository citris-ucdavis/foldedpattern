...
namespace RelatedObjects {
	...
	public class Main : MonoBehaviour {
        ...
        private void Awake()
        {
            //Used to get script attached to robot game object
        ...
        public void HandleObjectClick(GameObject go) {
			...
			foreach (GameObject ii in incorrectInstances) {
				if (go == ii) {
                    ...
                    //Rotate all boxes by 90 degrees to give new perspective...
                    //Coroutine to allow animation
                    StartCoroutine(RotateBox(incorrectInstances[0], 90));
                    StartCoroutine(RotateBox(incorrectInstances[1], 90));
                    StartCoroutine(RotateBox(incorrectInstances[2], 90));
                    StartCoroutine(RotateBox(correctInstance, 90));
                    return;
				}
			}
        ...
        //Roates "myObject" (make sure to say "using System.Collections;" at top of code)
        public IEnumerator RotateBox(GameObject myObject, int rotationWanted)
        {
            //Initial rotation needed is the ammount wanted
            int rotationNeeded = rotationWanted;

            //Initial orientation
            Vector3 myObjectOrientation = myObject.transform.localEulerAngles;
            
            //Initial orientation in X,Y,Z
            int X = (int)myObjectOrientation.x;
            int Y = (int)myObjectOrientation.y;
            int Z = (int)myObjectOrientation.z;

            //While rotation is needed
            while (rotationNeeded != 0)
            {
                //Decrease Z (assumed Z axis rotation) and decrease counter
                Z = Z - 1;
